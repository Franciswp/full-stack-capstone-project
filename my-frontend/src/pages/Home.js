...
function getPosterUrl(movie) {
  ...
  const exact = Object.entries(posters).find(([p]) =>
    p.toLowerCase().endsWith(src.toLowerCase())
  );
  ...
  if (file) {
    const byFile = Object.entries(posters).find(([p]) =>
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const fetchMovies = async () => {
      try {
        setLoading(true);
        setError(null);
        const res = await api.get('/movies/all', { signal: controller.signal });
  ...
  const filtered = useMemo(() => {
    ...
    return movies.filter((m) =>
      (m.title ?? '').toString().toLowerCase().includes(q)
    );
  }, [movies, query]);
  ...
  const skeletonIds = useMemo(() =>
    Array.from({ length: 8 }, (_, i) => `skeleton-${i}-load`)
  , []);
      ...
      {!loading && hasData && (
        <Grid container spacing={3}>
          {sorted.map((movie) => {
            ...
            return (
              <Grid key={String(movie.id)} item xs={12} sm={6} md={4} lg={3}>
                <Card
                  sx={{
                    height: '100%',
                    display: 'flex',
